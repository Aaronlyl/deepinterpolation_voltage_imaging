## Edited for voltage imaging data
...
class OphysGenerator(SequentialGenerator):
    ...
    frame is expected to be smaller than (128,128)."""
    ...
    def __getitem__(self, index):
        ...
        input_full = np.zeros(
            [self.batch_size, 128, 128, self.pre_frame + self.post_frame],
            dtype="float32",
        )

        output_full = np.zeros([self.batch_size, 128, 128, 1], dtype="float32")
    ...
    def __data_generation__(self, index_frame):
        ...
        input_full = np.zeros([1, 128, 128, self.pre_frame + self.post_frame])
        output_full = np.zeros([1, 128, 128, 1])
...
class MovieJSONGenerator(DeepGenerator):
    ...
    def __getitem__(self, index):
        ...
        output_full = np.zeros([self.batch_size, 128, 128, 1])
    ...
    def __data_generation__(self, index_frame):
        ...
        try:
            ...
            motion_path = []
            if os.path.exists(local_path) and not os.path.islink(local_path):  # Path exists and is not symbolic
            ...
            input_full = np.zeros(
                [1, 128, 128, self.pre_frame + self.post_frame])
            output_full = np.zeros([1, 128, 128, 1])
